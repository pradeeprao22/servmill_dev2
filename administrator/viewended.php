...
<body>
	<div id="container">
        ...
        <tbody>
        	<?php
			  $bids_stat = mysql_query("SELECT * FROM services WHERE status = 0") or die(mysql_error());
			  WHILE($stat = mysql_fetch_array($bids_stat)){
				$prodid = $stat['serviceid'];
				$numbidderq = mysql_query("Select * FROM servicereport WHERE serviceid = '$prodid'")or die(mysql_error());
				$numbidder = MYSQL_NUMROWS($numbidderq);
				echo
					"<tr>
            <td align='center'>".$stat['servicename']."</td>
