	...
	<div id="container">
		<div id="bgwrap">
			...
			<center><h1><br />Welcome Administrator</h1></center>
				...
				$bidnum = mysql_query("SELECT * FROM bidreport LEFT JOIN member ON member.memberid = bidreport.bidder LEFT JOIN services ON services.serviceid = bidreport.serviceid WHERE bidreport.status = 0") or die(mysql_error());
						$count = 0;
				...
				$endedsum = mysql_query("SELECT * FROM services WHERE status = 1") or die(mysql_error());
						$counters = 0;
				...
				<!--end query-->
					...
					<?php $bids_stat = mysql_query("SELECT * FROM bidreport LEFT JOIN member ON member.memberid = bidreport.bidder LEFT JOIN services ON services.serviceid = bidreport.serviceid WHERE bidreport.status = 0") or die(mysql_error());
					...
					?> 
							...
							Has Placed <strong>Php <?php echo $stat['bidamount']; ?></strong> On <?php echo $stat['servicename']; ?> last <strong><?php echo $stat['biddatetime']; ?></strong>
							</p>
							</div>
							<?php } ?>
					<!--end of notification-->
					...
					<!--query for notifications-->
					<?php
					    $endcheck = mysql_query("SELECT * FROM services WHERE status = 1") or die(mysql_error());
						WHILE($static = mysql_fetch_array($endcheck)){
						$prodid = $static['serviceid'];
						$prodname = $static['servicename'];
						$query2 = mysql_query("SELECT * FROM bidreport WHERE serviceid = '$prodid'") or die (mysql_error());
						...
						if ($numberOfRows == 0)
							...
							while($row2 = mysql_fetch_array($query2)){
								...
								$query3 = mysql_query("SELECT * FROM bidreport WHERE bidamount = '$higestbid' AND serviceid = '$prodid' ") or die (mysql_error());
								$winner=mysql_fetch_array($query3);
								$hewins=$winner['bidder'];
								mysql_query("UPDATE services SET status = 0 WHERE serviceid = '$prodid' ") or die (mysql_error());
								$rows = mysql_query("SELECT * needtopay WHERE memberid = '$hewins' AND serviceid = '$prodid'");
								$numrows1 = MSQL_NUMROWS($rows);
								if ($numrows1 == 0){
								mysql_query("INSERT INTO needtopay(memberid,serviceid,status,payment,declined,dateadded) VALUES('$hewins','$prodid','0','$higestbid','0',NOW()) ") or die (mysql_error());
								}
							}
					?> 
							
							<div class="ended id<?php echo $static['serviceid']; ?>" id="<?php echo $static['serviceid']; ?>">
							<p>
							<br /><span><?php echo $servicename; ?></span>
