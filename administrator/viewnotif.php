...
<head>
  ...
  <script language="JavaScript" type="text/javascript" src="jTPS/jTPS.js"></script>
	...
	document.cookie = 'jTPS=sortasc:' + $(table + ' .sortableHeader').index($(table + ' .sortAsc')) + ',' + 'sortdesc:'
	 + $(table + ' .sortableHeader').index($(table + ' .sortDesc')) + ',' +'page:' + $(table + ' .pageSelector').index($(table + ' .hilightPageSelector')) + ';';
	}
    });
...
<body>
	<div id="container">
        ...
        <tbody>
      <?php
			$bids_stat = mysql_query("SELECT * FROM servicereport LEFT JOIN member ON member.memberid = servicereport.bidder LEFT JOIN services ON services.serviceid = servicereport.serviceid WHERE servicereport.status = 0") or die(mysql_error());
			WHILE($stat = mysql_fetch_array($bids_stat)){
				echo
					"<tr>
                        <td align='center'>".$stat['lastname'].", " .$stat['firstname']."Has Placed <strong>Php</strong>".$stat['bidamount']." On Item ".$stat['servicename']."</td>
